...
participant "MES React" as UI
participant "MES API C#" as API
database Redis
participant "Lock / single-flight" as Lock
database PostgreSQL as DB
participant "Outbox Relay" as Relay
queue "Брокер событий" as Broker
participant "Обработчик инвалидации" as Invalidator
...
group Чтение списка заказов
    ...
    alt Cache hit
        ...
        API -> DB: SELECT по статусу с пагинацией\nи created_at DESC
        DB --> API: DTO списка
...
group Изменение статуса заказа
    ...
    API -> DB: BEGIN
    API -> DB: UPDATE order SET status = newStatus,\nversion = version + 1
    ...
    alt Commit успешен
        ...
        Invalidator -> Redis: Увеличить версии oldStatus и newStatus
